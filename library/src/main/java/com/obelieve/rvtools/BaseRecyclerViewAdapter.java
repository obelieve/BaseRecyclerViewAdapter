...
import static com.obelieve.rvtools.BaseRecyclerViewAdapter.LoadMoreStatus.LOADING;
...
public abstract class BaseRecyclerViewAdapter<DATA> extends RecyclerView.Adapter<BaseRecyclerViewAdapter.BaseViewHolder> {
    ...
    private LoadMoreStatus mLoadMoreState = LoadMoreStatus.END;
    ...
    public void loadMoreError() {
        if (mOnLoadMoreListener == null)
            return;
        mLoadMoreState = LoadMoreStatus.ERROR;
        mLockLoadMore = false;
        if (getDataHolder().getList().size() > 0) {
            notifyItemChanged(getItemCount() - 1);
        }
    ...
    public void loadMoreEnd() {
        if (mOnLoadMoreListener == null)
            return;
        mLoadMoreState = LoadMoreStatus.END;
        mLockLoadMore = false;
        if (getDataHolder().getList().size() > 0) {
            notifyItemChanged(getItemCount() - 1);
        }
    ...
    public void onBindViewHolder(@NonNull BaseViewHolder holder, int position) {
        ...
        if (!(holder instanceof SimpleViewHolder) && !(holder instanceof LoadMoreViewHolder)) {
            ...
            if (holder instanceof LoadMoreViewHolder) {
                final LoadMoreViewHolder holder1 = (LoadMoreViewHolder) holder;
    ...
    public void onAttachedToRecyclerView(@NonNull final RecyclerView recyclerView) {
        ...
        if (recyclerView.getLayoutManager() instanceof GridLayoutManager) {
             ...
             * Grid布局，有特殊类型时显示一行
             */
            final GridLayoutManager.SpanSizeLookup lookup = ((GridLayoutManager) recyclerView.getLayoutManager()).getSpanSizeLookup();
